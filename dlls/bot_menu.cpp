...
void SetupMenus()
{
	...
	else if (gBotGlobals.IsMod(MOD_WW))
	{
		gBotGlobals.m_Menus[BOT_MENU_WAYPOINT_GIVE_FLAGS6].AddMenuItem(1, "Flag Point", BotMenu_Func_Sci_Waypoint);
		gBotGlobals.m_Menus[BOT_MENU_WAYPOINT_GIVE_FLAGS6].AddMenuItem(2, "Capture Point", BotMenu_Func_Barney_Waypoint);
	}
	else
	{
...
{
	for (CBotMenuItem* m_Menu : m_Menus)
	{
		if (m_Menu != nullptr)
			m_Menu->Init();
	}
