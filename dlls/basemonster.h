...
public:
	...
	// aone
	virtual void SetupFriendly() {}; // use this overridden to make some other friendly monsters
	// end aone
	...
	void Precache(){ CBaseEntity::Precache();};
...
public:
	...
	virtual void Move(float flInterval = 0.1f);
	virtual void MoveExecute(CBaseEntity* pTargetEnt, const Vector& vecDir, float flInterval);
	virtual BOOL ShouldAdvanceRoute(float flWaypointDist);
	...
	void TaskComplete() { if (!HasConditions(bits_COND_TASK_FAILED)) m_iTaskStatus = TASKSTATUS_COMPLETE; }
	void MovementComplete();
	void TaskFail() { SetConditions(bits_COND_TASK_FAILED); }
	void TaskBegin() { m_iTaskStatus = TASKSTATUS_RUNNING; }
	int TaskIsRunning();
	int TaskIsComplete() const { return (m_iTaskStatus == TASKSTATUS_COMPLETE); }
	int MovementIsComplete() const { return (m_movementGoal == MOVEGOAL_NONE); }
	...
	void SetConditions(int iConditions) { m_afConditions |= iConditions; }
	void ClearConditions(int iConditions) { m_afConditions &= ~iConditions; }
	BOOL HasConditions(int iConditions) const { if (m_afConditions & iConditions) return true; return false; }
	BOOL HasAllConditions(int iConditions) const { if ((m_afConditions & iConditions) == iConditions) return true; return false; }
	...
	virtual float HearingSensitivity() { return 1.0f; }
	...
	Vector ShootAtEnemy(const Vector& shootOrigin);
	Vector BodyTarget(const Vector& posSrc) override { return Center() * 0.75f + EyePosition() * 0.25f; }
	// position to shoot at
	...
	void	Remember(int iMemory) { m_afMemory |= iMemory; }
	void	Forget(int iMemory) { m_afMemory &= ~iMemory; }
	BOOL HasMemory(int iMemory) const { if (m_afMemory & iMemory) return true; return false; }
	BOOL HasAllMemories(int iMemory) const { if ((m_afMemory & iMemory) == iMemory) return true; return false; }
